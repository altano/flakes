...
| `defaultCatalog.steam.enableProtonLogging` | bool               | `true`                | Set `PROTON_LOG=1` on the built-in Steam app (mirrors upstream Wolf). `false` omits it, avoiding a multi-MB/s `trace:unwind` log flood that drops exception-heavy games (most Unity/Mono titles) to <1fps. |
